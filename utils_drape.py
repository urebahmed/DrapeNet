...
def load_udf(checkpoints_dir, code_file_name, model_file_name, device):
    ...
    decoder.load_state_dict(torch.load(os.path.join(checkpoints_dir, model_file_name), map_location=torch.device('cpu'))["decoder"])
...
def load_lbs(checkpoints_dir, device):
    ...
    model_lbs.load_state_dict(torch.load(os.path.join(checkpoints_dir, 'lbs.pth'), map_location=torch.device('cpu')))

    model_lbs_shape = drapenet.skip_connection(d_in=10+3, d_out=3, width=256, depth=8, skip_layer=[4]).to(device).eval()
    model_lbs_shape.load_state_dict(torch.load(os.path.join(checkpoints_dir, 'lbs_shape.pth'), map_location=torch.device('cpu')))
    ...
    model_lbs_deform_top.load_state_dict(torch.load(os.path.join(checkpoints_dir, 'lbs_deform_top.pth'), map_location=torch.device('cpu')))
    ...
    model_lbs_deform_bottom.load_state_dict(torch.load(os.path.join(checkpoints_dir, 'lbs_deform_bottom.pth'), map_location=torch.device('cpu')))
    ...
    model_lbs_deform_layer.load_state_dict(torch.load(os.path.join(checkpoints_dir, 'lbs_deform_layer.pth'), map_location=torch.device('cpu')))
