...
class GridFiller:
    ...
    def __init__(
        ...
        for i, N in enumerate(self.N_levels):
            ...
            self.masks_coarse[i] = mask_coarse.clone()
            ...
            self.idxs_coarse_neighbors_blocks[
                i
            ] = idxs_coarse_neighbors_blocks.clone()
            ...
            if i > 0:
                ...
                self.masks_coarse_no_recompute[
                    i
                ] = mask_coarse_no_recompute.clone()
    ...
    def fill_grid(
        ...
        with torch.no_grad():
            ...
            for level, N in enumerate(self.N_levels):
                ...
                """ Query the network """
                xyz = samples[mask_coarse_no_recompute, 0:3]
                # Query and fill grid.
...
def sample_udf(
    ...
    udf = torch.zeros(coords.shape[0])
    start = 0
...
def sample_grads(
    ...
    grads = torch.zeros(coords.shape[0], 3)
    start = 0
...
def get_udf_and_grads(
    ...
    samples = torch.cat([coords, zeros], dim=-1)
...
def get_mesh_from_udf(
    ...
    final_verts = torch.tensor(mesh.vertices).float()
    final_faces = torch.tensor(mesh.faces).long()
    ...
    if differentiable:
        ...
        edge_border = torch.tensor(edge_border).float()
        ...
        border_verts = torch.tensor(mesh.vertices[u_border]).float()
